...
![Presentación del Polar Chart](./img/polar_chart.gif)

## Descripción General

El componente Lightning Web Component (LWC) 'Polar Chart' ofrece una herramienta de visualización dinámica e interactiva para mostrar indicadores de rendimiento dentro de Salesforce. Este componente está diseñado para representar datos en forma de gráfico circular, proporcionando a los usuarios una manera clara e intuitiva de evaluar y comparar los valores actuales con los objetivos. Al utilizar el Polar Chart, los usuarios pueden monitorear eficazmente los indicadores clave de rendimiento (KPI) y tomar decisiones informadas basadas en insights visuales.

## ¿Cómo Funciona?

El Polar Chart muestra los datos en un gráfico circular, con segmentos que representan diferentes métricas. El gráfico incluye:
...
Para usar el Polar Chart, debe configurar un flujo en Salesforce que recupere los datos necesarios y los pase al LWC. Aquí le mostramos cómo hacerlo:
...
Alternativamente, puede usar consultas de entrada para proporcionar datos al Polar Chart. Aquí le mostramos cómo:
...
2. **Pasar las Consultas de Entrada al Componente**:
   - Use el atributo `inputQueries` del componente Polar Chart para pasar la cadena JSON de las consultas de entrada.
   - El componente ejecutará esta consulta y usará los resultados para llenar el gráfico.

3. **Crear una Función de Transformación**:
   - Defina una función de transformación JavaScript en un archivo y súbala como un Recurso Estático en Salesforce. Esta función procesará los resultados de las consultas y los formateará para el Polar Chart.
   - Ejemplo:
     ```javascript
